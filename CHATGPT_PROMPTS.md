...
- これは**0.6.0の履歴上限**として`MAX_SPEAKERS=16`、`MAX_SOURCES=128`、`MAX_ROUTES_GLOBAL=512`、`MAX_ROUTES_PER_SOURCE=16`を採用した。0.7.0以降の実装では、後半の「G. 0.7.0」節にある`MAX_ROUTES_GLOBAL=2048`とGate A〜Rを必ず優先する。
...
0.7.0の唯一の実装判断正本はARCHITECTURE_GATE_0_7.mdです。Gate A〜Rが最終レビューでGOとなるまで、コードを書いてはいけません。
...
作業順序:
1. Gate A〜Rの要件を満たすProcessor editing API、pending automation mailbox、revision transaction、route slot lifecycleの設計を提示し、承認を待つ。
